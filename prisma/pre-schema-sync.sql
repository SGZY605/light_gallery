...
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'Share'
  ) AND EXISTS (
    SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'Image'
  ) THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM pg_constraint WHERE conname = 'ShareImage_shareId_fkey'
    ) THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM pg_constraint WHERE conname = 'ShareImage_imageId_fkey'
    ) THEN
      ALTER TABLE "ShareImage"
